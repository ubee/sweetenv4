...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    this._user.getclients()
    .subscribe(
      data=>this.addData(data),
      error=>this._router.navigate(['/login'])
    )

    this._user.getcontractors()
    .subscribe(
  ...
  count(data){
    this.homeprojs=data
    this.homeprojs.forEach(element => {
     this.counter++;
     });
  ...
  counter1(data){
    this.businessprojs=data
    this.businessprojs.forEach(element => {
     this.counter2++;
     });
  ...
  navigate1(){
    this._router.navigate(['/clientprofile'])
  }
  navigate2(){
